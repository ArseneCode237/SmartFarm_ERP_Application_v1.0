...
public class MouvementStockService {
    ...
    private BigDecimal calculerStockApres(BigDecimal stockAvant, MouvementRequest request) {
        return switch (request.getTypeMouvement()) {
            case ENTREE -> stockAvant.add(request.getQuantite());
            case SORTIE, PERTE -> stockAvant.subtract(request.getQuantite());
            case AJUSTEMENT -> request.getQuantite();
            case TRANSFERT -> stockAvant; // le stock global est inchangé; le détail des entrepôts est porté par destinationOrigine.
        };
    }
    ...
    public static class MouvementStatistiques {
        ...
        MouvementStatistiques(List<MouvementStock> m) {
            total = m.size(); totalEntrees = count(m, TypeMouvementStock.ENTREE); totalSorties = count(m, TypeMouvementStock.SORTIE);
            totalAjustements = count(m, TypeMouvementStock.AJUSTEMENT); totalPertes = count(m, TypeMouvementStock.PERTE);
            valeurEntrees = value(m, TypeMouvementStock.ENTREE); valeurSorties = value(m, TypeMouvementStock.SORTIE);
        }
